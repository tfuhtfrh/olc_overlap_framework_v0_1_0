"""QBSolv benchmark for latent strict-rank comparator QUBO."""
...
OUT=Path("debug/qubo/chm13_latent_rank_comparator_qbsolv_20260926.json")
SIZES=[47,100,200]
SEED=20260926

def main():
    m,reward,cfg=build_model(LatentRankComparatorConfig())
    ref=json.loads((DATASET_DIR/"reference_path.json").read_text())["normalized_nodes"]
    rs=encode_order(m,ref)
    refE=m.energy(rs)
    rows=[]
